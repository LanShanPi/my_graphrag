...
# llm = OpenAI(temperature=0, model="gpt-4", device=device)
...
def generate_subgraph(index, store_dir, noun=None):
    ...
    # 创建存储上下文
    storage_context = StorageContext.from_defaults(graph_store=index.property_graph_store)

    # 初始化 PropertyGraphIndex
    subgraph_index = PropertyGraphIndex(nodes=nodes, storage_context=storage_context)

    # 保存子图索引（由于使用Neo4j，不需要额外保存）
    subgraph_dir = os.path.join(store_dir, "subgraph")
